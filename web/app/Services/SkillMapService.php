...
class SkillMapService extends BaseService
{
    /** @var Model */
    private $model;
    ...
    public function __construct(SkillMap $model)
    {
        $this->model = $model;
    ...
    public function getListMap(Request $request)
    {
        ...
        return $this->model::select("skillmaps.*")->orderBy('skillmaps.id')
        ->join('departments', 'departments.id', '=', 'skillmaps.department_id')
        ->with('department:id,name')->where($this->keyUserId, $userId)->paginate($limit);
    }
     ...
     * @return object
     */
    public function destroyListMap($id)
    {
        $data = $this->model::find($id);
        $result = null;
        if ($data) {
            $skillMapId = $data['id'];
            $result = $this->destroy($skillMapId);
        }
        if ($result) {
            $data->delete();
        }
